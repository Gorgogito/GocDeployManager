...
{
    /// <summary>
    /// Exportación a Excel y PDF (ambos formatos confirmados con el cliente,
    /// sección 5 del análisis). Excel usa ClosedXML (tabla real); PDF usa
    /// PdfSharp con un reporte de texto simple, no una grilla pixel-perfect.
    /// </summary>
        ...
        public static void ExportarPdf(IReadOnlyList<Despliegue> despliegues, string rutaArchivo)
        {
            using (var documento = new PdfDocument())
            {
                ...
                void NuevaPagina()
                {
                    ...
                    g.DrawString("Historial de despliegues — GocDeployManager", fuenteTitulo, XBrushes.Black, margen, margen);
                    y = margen + 26;
                }
                ...
                foreach (var d in despliegues)
                {
                    ...
                    g.DrawString(
                        $"{d.FechaHora:yyyy-MM-dd HH:mm:ss}  ·  {d.Goc}  ·  {d.Ambiente}  ·  {string.Join(", ", d.Sistemas)}  ·  {d.Resultado}",
                        fuenteEncabezado, XBrushes.Black, margen, y);
                    y += alturaLinea;

                    g.DrawString(
                        $"Usuario: {d.UsuarioAplicacion} ({d.UsuarioWindows}@{d.Equipo})  ·  Compilación: {d.TiempoCompilacion:hh\\:mm\\:ss}  ·  Despliegue: {d.TiempoDespliegue:hh\\:mm\\:ss}",
