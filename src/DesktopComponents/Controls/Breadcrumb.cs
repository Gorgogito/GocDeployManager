    ...
    public sealed class Breadcrumb : Control, IThemedControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (var i = 0; i < _segmentos.Count; i++)
            {
                ...
                var color = esUltimo ? tema.TextoPrimario : (hover ? tema.Acento : tema.TextoSecundario);

                TextRenderer.DrawText(g, texto, Font, rect, color, TextFormatFlags.VerticalCenter | TextFormatFlags.Left);

                if (hover)
                {
                    using (var lapiz = new Pen(tema.Acento))
                        g.DrawLine(lapiz, rect.Left, rect.Bottom - 2, rect.Right, rect.Bottom - 2);
                }
                ...
                if (!esUltimo)
                {
                    ...
                    TextRenderer.DrawText(g, separador, Font, new Rectangle(x, 0, tamSep.Width, Height),
